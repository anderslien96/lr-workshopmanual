...
function AIChatPanel({ prefill, onPrefillConsumed, onManualLink, manualData, onClose }) {
  ...
  const send = async (overrideText, overridePdfKey) => {
    ...
    try {
      // Build system prompt — inject PDF text if available
      ...
      setError("Connection failed — make sure the server is running and try again.");
    } finally {
      setLoading(false);
    }
  };
...
export default function App() {
  ...
  // Load manual JSON once on startup
  useEffect(() => {
  ...
  // When AI suggests a manual link, navigate to it and highlight
  ...
  const AskBtn = ({ folder, sub, file, label, small }) => {
    ...
    return (
      <button
        ...
        title={hasContent ? "AI will read the actual manual procedure" : "AI will answer from general knowledge"}
      >
    ...
    <div style={{ fontFamily: "'Inter', system-ui, sans-serif", background: "#060a12", height: "100vh", color: "#e2e8f0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {manualError && (
        <div style={{ background: "#1a0a00", borderBottom: "1px solid #fb923c44", padding: "10px 20px", display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "#fb923c" }}>
          <span>⚠️</span>
          <span>Server not running — PDFs and AI manual lookup won't work.</span>
          <span style={{ color: "#475569" }}>Double-click <strong style={{ color: "#fb923c" }}>START SERVER - Double Click Me.bat</strong> then refresh this page.</span>
        </div>
      )}
      ...
      <div style={{ background: "#0a0f1e", borderBottom: "1px solid #111827", padding: "12px 20px", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: "#334155", marginTop: 2 }}>
            {manualLoading ? "Loading manual data…" : manualError ? "Server offline" : `${totalPDFs} procedures · ${Object.keys(manualData || {}).length} with AI content · 5 sections`}
          </div>
        </div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", background: "#0c1220", border: `1px solid ${manualError ? "#ef444430" : "#34d39930"}`, borderRadius: 8 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: manualError ? "#ef4444" : "#34d399", boxShadow: manualError ? "0 0 6px #ef4444" : "0 0 6px #34d399" }} />
            <span style={{ fontSize: 11, color: manualError ? "#ef4444" : "#34d399", fontWeight: 600 }}>
              {manualError ? "Server offline" : "Server online"}
            </span>
          </div>
                  ...
                  return (
                    <div key={i} style={{ background: "#0a0f1e", border: "1px solid #111827", borderRadius: 10, padding: "13px 14px", transition: "border-color 0.15s" }}
                      ...
                      <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 8 }}>
                        ...
                        {r.sub && <span style={{ fontSize: 10, color: "#334155", padding: "2px 6px" }}>{r.sub}</span>}
              ...
              {!aiOpen && (
                <div onClick={() => setAiOpen(true)}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#334155", marginTop: 3 }}>Describe your symptoms to the AI Mechanic — it reads your manual and helps you diagnose, then points you to the right procedure.</div>
                  </div>
          ...
          {!search && selectedSection && !selectedFolder && (
            <div style={{ animation: "fadeIn 0.2s ease" }}>
              <div style={{ fontSize: 12, color: "#475569", marginBottom: 18, display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ cursor: "pointer" }} onClick={() => setSelectedSection(null)}
                  onMouseEnter={e => e.currentTarget.style.color = "#94a3b8"}
                  onMouseLeave={e => e.currentTarget.style.color = "#475569"}>All Sections</span>
                ...
                {(bySection[selectedSection] || []).map(({ folder, label, files }) => (
                  <div key={folder} onClick={() => setSelectedFolder(folder)}
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      <button onClick={e => { e.stopPropagation(); askAI(`I need help with the "${label}" system (${folder.split(" ")[0]}) on my Discovery 3. What are the most common failure points, symptoms to look for, and which procedures are most likely to be needed?`, null); }}
                        style={{ fontSize: 10, padding: "3px 8px", background: "#34d39915", border: "1px solid #34d39940", borderRadius: 5, color: "#34d399", cursor: "pointer", fontWeight: 600, transition: "all 0.15s" }}
            ...
            <div style={{ animation: "fadeIn 0.2s ease" }}>
              <div style={{ fontSize: 12, color: "#475569", marginBottom: 18, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                <span style={{ cursor: "pointer" }} onClick={() => setSelectedSection(null)}
                  ...
                  onMouseLeave={e => e.currentTarget.style.color = "#475569"}>All Sections</span>
                <span style={{ color: "#1e2a3a" }}>›</span>
                <span style={{ cursor: "pointer" }} onClick={() => setSelectedFolder(null)}
                  onMouseEnter={e => e.currentTarget.style.color = "#94a3b8"}
                  onMouseLeave={e => e.currentTarget.style.color = "#475569"}>{secInfo.icon} {secInfo.name}</span>
                <span style={{ color: "#1e2a3a" }}>›</span>
                      ...
                      {items.map(({ sub, file }, idx) => {
                        ...
                        return (
                          <div key={idx} ref={isHighlighted ? highlightRef : null}
                            style={{ background: isHighlighted ? secInfo.color + "15" : "#0a0f1e", border: `1px solid ${isHighlighted ? secInfo.color : secInfo.color + "20"}`, borderRadius: 10, padding: "11px 13px", display: "flex", alignItems: "center", gap: 10, transition: "all 0.15s", animation: isHighlighted ? "fadeIn 0.4s ease" : undefined }}
                            ...
                            <div style={{ flex: 1, fontSize: 12, color: "#e2e8f0", fontWeight: 500, minWidth: 0, lineHeight: 1.4 }}>
                              {file}
                              {hasContent && <span style={{ display: "inline-block", width: 5, height: 5, borderRadius: "50%", background: "#34d399", marginLeft: 6, verticalAlign: "middle", opacity: 0.7 }} title="AI can read this procedure" />}
                            </div>
